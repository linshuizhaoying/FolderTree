...
module.exports = {
    ...
    listFolder: function(level, folderName, exceptArr) {
        ...
        listArr.push(_level)
        listArr.push(this._getFolderStruct(folderName,_level, exceptArr))
    ...
    _getFolderStruct: function(dir,level,exceptArr) {
        ...
        function getMap(dir, curIndex) {
            ...
            files.map(function(file) {
                ...
                try {
                    ...
                    if (stats.isDirectory() && mapDeep[file] <level) { //判断是否为文件夹类型
    ...
    _watcher: function(folderName,writeFileName) {
        chokidar.watch(folderName, {ignored: /(^|[\/\\])\../}).on('all', (event, path) => {
          //console.log(event, path);
          this.listFolder(preLevel,prefolderName,preexceptArr);
          this.writeFile(writeFileName);
         // console.log(writeMap);
        });

    },
    writeFile:function(writeFileName){
        fs.open(writeFileName,"w+",function(err,fd){
            var buf = new Buffer(writeMap);
